...
switch ($method) {
    ...
    case "POST":
        ...
        if (isset($_FILES['pimage'])) {
            ...
            $p_tmp_image    = $_FILES["pimage"]["tmp_name"];
            $destination = $_SERVER["DOCUMENT_ROOT"]."/phpapiforcrud/product_image/$p_image";
        ...
        if (!empty($product_name) && !empty($price) && !empty($p_image) && !empty($status)) 
        {
                    ...
                    if ($result) {
                        ...
                        move_uploaded_file($p_tmp_image,$destination);
        ...
        echo json_encode(["success"=>"Please Choose Product Image File."]);
